...
with open('BLACKBOX_INPUT/input.txt', 'w') as input_f:
  pathlist = Path('../CodeInquisitor/PATTERN_ANALYSIS/blackbox_time_series_Jan_July_2019_w_timestamps').glob('*/*')
    ...
    try:
      with open(path_in_str, 'r') as f:
        ...
        for event in content:
          if event in ('22', '19', '20', '8', '9') and not ON_REPETITIVE_EVENT:
            ON_REPETITIVE_EVENT = True
            aggregated_content.append(event)
          elif event in ('22', '19', '20', '8', '9') and ON_REPETITIVE_EVENT:
            pass
          else:
            ON_REPETITIVE_EVENT = False
            aggregated_content.append(event)
